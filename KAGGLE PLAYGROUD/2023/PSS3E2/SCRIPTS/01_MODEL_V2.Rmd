...
## TUNEO DE MODELOS Y VALIDACIÓN CRUZADA

```{r}
rf_spec_tune <- rand_forest(trees = tune(), 
                           mtry = tune()) %>% 
  set_engine("ranger", 
             seed = 42, 
             verbose = TRUE) %>% 
...
rf_fit_tune <- tune_grid(rf_wf_tune, 
                         resamples = folds, 
                         grid = crossing(trees = c(10, 50, 100, 200, 500, 1000), 
                                         mtry = c(2, 4, 6, 10, 12, 14, 16)), 
...
rf_fit_final %>% 
  ...
  write_csv("OUTPUTS/SUBMIT_V2.csv")
