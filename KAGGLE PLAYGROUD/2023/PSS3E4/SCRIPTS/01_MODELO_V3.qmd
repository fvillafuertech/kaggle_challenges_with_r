...
dataset_p <- dataset %>% 
  ...
  # select(-time)
  ## Feature Engineering
  # mutate(sum = apply(select(., contains("v"), amount), 
  #                    MARGIN = 1, 
  #                    sum), 
  #        mean = apply(select(., contains("v"), amount), 
  #                     MARGIN = 1, 
  #                     mean), 
  #        sd = apply(select(., contains("v"), amount), 
  #                   MARGIN = 1, 
  #                   sd))
...
particion_inicial <- initial_split(dataset_p, prop = 0.8, strata = class)
dfTrain <- training(particion_inicial)
dfTest <- testing(particion_inicial)

folds <- vfold_cv(dfTrain, v = 5, strata = class)
```
...
xgb_fit_tune <- tune_grid(xgb_wf, 
                          resamples = folds, 
                          grid = crossing(trees = c(10, 50, 100, 200, 500, 600), 
                                          mtry = c(2, 4, 8, 10, 12, 15, 20, 24, 29)), 
...
xgb_fit <- xgb_wf %>% 
  finalize_workflow(select_best(xgb_fit_tune)) %>% 
  fit(dfTrain)
...
lgbm_fit_tune <- tune_grid(lgbm_wf, 
                           resamples = folds, 
                           grid = crossing(trees = c(500, 510, 520, 530, 540, 550), 
                                           mtry = c(2, 4, 8, 10, 12, 15, 20, 24, 29)), 
                           metrics = metric_set(roc_auc), 
...
lgbm_fit <- lgbm_wf %>% 
  finalize_workflow(select_best(lgbm_fit_tune)) %>% 
  fit(dfTrain)
```
...
submit %>% 
  ...
  write_csv("OUTPUTS/SUBMIT_XGB_V3.csv")
...
submit %>% 
  ...
  write_csv("OUTPUTS/SUBMIT_LGBM_V3.csv")
